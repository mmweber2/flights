...
def search_flights(config_file, recipient, key_path, pw_path):
    ...
    if "MAX_FLIGHTS" in config:
        # Combine the best sub results from (possibly) multiple queries
...
def _parse_config_file(config_file):
    ...
    # See search_flights for config_file format requirements.
    config_info = []
...
def send_request(query, key_path):
    ...
    Limited by the QPX system to 50 queries per day.
